...
class AttendrixSupabaseUser extends BaseAuthUser {
  AttendrixSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> attendrixSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = AttendrixSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
