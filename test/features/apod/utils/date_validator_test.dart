...
void main() {
  group('DateValidator Tests', () {
    ...
    test('getValidationError returns correct message for empty input', () {
      expect(DateValidator.getValidationError(''), equals('Please enter a date'));
    });
    ...
    test('getValidationError returns correct message for invalid months/days', () {
      expect(
    ...
    test('getValidationError returns correct message for dates before June 16, 1995', () {
      expect(
  ...
  group('DateTextInputFormatter Tests', () {
    ...
    test('adds hyphens as digits are typed', () {
      ...
      // Type "2026"
      var val = formatter.formatEditUpdate(oldValue, const TextEditingValue(text: '2026'));
      expect(val.text, equals('2026'));

      // Type "20260" -> should add hyphen: "2026-0"
      val = formatter.formatEditUpdate(val, const TextEditingValue(text: '20260'));
      expect(val.text, equals('2026-0'));

      // Type "2026-05"
      val = formatter.formatEditUpdate(val, const TextEditingValue(text: '2026-05'));
      expect(val.text, equals('2026-05'));

      // Type "2026-052" -> should add hyphen: "2026-05-2"
      val = formatter.formatEditUpdate(val, const TextEditingValue(text: '2026-052'));
      expect(val.text, equals('2026-05-2'));

      // Type "2026-05-22"
      val = formatter.formatEditUpdate(val, const TextEditingValue(text: '2026-05-22'));
      expect(val.text, equals('2026-05-22'));
    });

    test('gracefully handles deletes/backspaces', () {
      const oldValue = TextEditingValue(text: '2026-0', selection: TextSelection.collapsed(offset: 6));
      
      // User presses backspace to delete '0' -> text becomes "2026-"
      final val = formatter.formatEditUpdate(oldValue, const TextEditingValue(text: '2026-'));
      expect(val.text, equals('2026-'));
    });

    test('blocks inputs longer than 8 digits', () {
      const oldValue = TextEditingValue(text: '2026-05-22', selection: TextSelection.collapsed(offset: 10));
      
      // Try typing an extra digit "2" at the end -> "2026-05-222"
      final val = formatter.formatEditUpdate(oldValue, const TextEditingValue(text: '2026-05-222'));
      expect(val.text, equals('2026-05-22'));
    });
  });
}
