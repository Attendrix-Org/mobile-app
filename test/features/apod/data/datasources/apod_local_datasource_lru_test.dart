...
void main() {
  ...
  group('ApodLocalDatasource LRU Eviction & Date Caching Tests', () {
    const dto1 = ApodDto(
      date: '2026-05-19',
      title: 'Entry 1',
      explanation: 'These pillars of gas and dust are being sculpted from within.',
      mediaType: 'image',
    ...
    test('getCachedApodForDate should return DTO and update LRU order/timestamps', () async {
      final date = DateTime(2026, 5, 20);
      const dateStr = '2026-05-20';

      when(() => mockPrefs.getString('apod_cache_entry_$dateStr'))
          .thenReturn(jsonEncode(dto2.toJson()));
      when(() => mockPrefs.getStringList('apod_cache_dates_lru'))
      ...
      verify(() => mockPrefs.setStringList('apod_cache_dates_lru', ['2026-05-19', '2026-05-20'])).called(1);
      verify(() => mockPrefs.setString(any(that: startsWith('apod_cache_timestamp_')), any())).called(1);
    });

    test('cacheApodForDate should add entry, update LRU list, and evict oldest if limit exceeded', () async {
      // 1. Mock first 3 cache entries added successfully
      var lruList = <String>[];
      
      when(() => mockPrefs.getStringList('apod_cache_dates_lru'))
          .thenAnswer((_) => lruList);
      when(() => mockPrefs.setStringList('apod_cache_dates_lru', any()))
      ...
      verify(() => mockPrefs.remove('apod_cache_timestamp_2026-05-19')).called(1);
    ...
    test('clearCache should remove all cache entries and helper keys', () async {
      when(() => mockPrefs.getStringList('apod_cache_dates_lru'))
          .thenReturn(['2026-05-20', '2026-05-21']);
      when(() => mockPrefs.remove(any()))
          .thenAnswer((_) async => true);
      ...
      verify(() => mockPrefs.remove('apod_cache_timestamp_2026-05-20')).called(1);
      verify(() => mockPrefs.remove('apod_cache_entry_2026-05-21')).called(1);
      verify(() => mockPrefs.remove('apod_cache_timestamp_2026-05-21')).called(1);
