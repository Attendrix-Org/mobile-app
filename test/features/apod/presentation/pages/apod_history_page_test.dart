...
void main() {
  ...
  setUp(() {
    ...
    when(() => mockPrefs.getStringList('apod_recent_searches')).thenReturn(null);
  ...
  group('ApodHistoryPage Widget Tests', () {
    ...
    testWidgets('explore today button navigates to detail page with null arguments', (tester) async {
      when(() => mockPrefs.getStringList('apod_viewed_dates')).thenReturn([]);
      ...
      await tester.pump();
      await tester.pump(const Duration(milliseconds: 100)); // Resolve today fetch
      await tester.pump(const Duration(milliseconds: 350)); // Navigation transition
      await tester.pump(const Duration(milliseconds: 600)); // Image load & animations on Detail Page
      await tester.pump();
    ...
    testWidgets('toggles history list and grid view mode', (tester) async {
      when(() => mockPrefs.getStringList('apod_viewed_dates')).thenReturn(['2026-05-21', '2026-05-20']);
      when(() => mockRepo.getApodForDate(DateTime(2026, 5, 21), forceRefresh: any(named: 'forceRefresh')))
          .thenAnswer((_) async => const Success(testEntry1));
      when(() => mockRepo.getApodForDate(DateTime(2026, 5, 20), forceRefresh: any(named: 'forceRefresh')))
      ...
      await tester.pump(const Duration(milliseconds: 350)); // Resolve animations
      await tester.pump();
    ...
    testWidgets('displays validation error for invalid search formats', (tester) async {
      when(() => mockPrefs.getStringList('apod_viewed_dates')).thenReturn([]);
    ...
    testWidgets('triggers successful search, adds to viewed/recent, and navigates to Detail Page', (tester) async {
      when(() => mockPrefs.getStringList('apod_viewed_dates')).thenReturn([]);
      when(() => mockRepo.getCachedApod(any()))
      ...
      await tester.pump(const Duration(milliseconds: 350)); // Navigator transition
      await tester.pump(const Duration(milliseconds: 600)); // Resolve image loads & animations on Detail Page
      await tester.pump();
      ...
      verify(() => mockPrefs.setStringList('apod_viewed_dates', ['2026-05-20'])).called(1);
      verify(() => mockPrefs.setStringList('apod_recent_searches', ['2026-05-20'])).called(1);
    ...
    testWidgets('renders recent searches and handles chip interaction and clearing', (tester) async {
      when(() => mockPrefs.getStringList('apod_recent_searches')).thenReturn(['2026-05-20', '2026-05-21']);
      when(() => mockPrefs.getStringList('apod_viewed_dates')).thenReturn([]);
      when(() => mockRepo.getApodForDate(any(), forceRefresh: any(named: 'forceRefresh')))
          .thenAnswer((_) async => const Success(testEntry1));
      ...
      await tester.tap(find.byKey(const Key('recent_search_chip_2026-05-20')));
      await tester.pump();
      await tester.pump(const Duration(milliseconds: 100)); // Future resolves
      await tester.pump(const Duration(milliseconds: 350)); // Detail page transition
      await tester.pump(const Duration(milliseconds: 600)); // Image load & animations on Detail Page
      await tester.pump();
      ...
      expect(find.byKey(const Key('recent_search_chip_2026-05-20')), findsNothing);
      expect(find.byKey(const Key('recent_search_chip_2026-05-21')), findsNothing);
    ...
    testWidgets('renders error state and retries on button tap', (tester) async {
      when(() => mockPrefs.getStringList('apod_viewed_dates')).thenReturn(['2026-05-21']);
      ...
      await tester.pump();
      await tester.pump(const Duration(milliseconds: 100)); // Resolve futures
      await tester.pump(const Duration(milliseconds: 350)); // Settle transitions
      await tester.pump();
  ...
  group('CalendarExplorer and Panel Integration Tests', () {
    testWidgets('toggles calendar view and triggers search on valid date selection', (tester) async {
      when(() => mockPrefs.getStringList('apod_viewed_dates')).thenReturn([]);
      when(() => mockRepo.getApodForDate(any(), forceRefresh: any(named: 'forceRefresh')))
          .thenAnswer((_) async => Success(testEntry1.copyWith(date: '2026-05-18')));
      ...
      await tester.pump(const Duration(milliseconds: 100)); // Future resolves
      await tester.pump(const Duration(milliseconds: 350)); // Detail page transition
      await tester.pump(const Duration(milliseconds: 600)); // Detail page loaded resources
      await tester.pump();
    ...
    testWidgets('calendar explorer disables dates before start date (June 16, 1995)', (tester) async {
      when(() => mockPrefs.getStringList('apod_viewed_dates')).thenReturn([]);
      ...
      final container = ProviderScope.containerOf(tester.element(find.byType(ApodHistoryPage)));
      container.read(calendarMonthYearProvider.notifier).state = const MonthYear(5, 1995);
      await tester.pump(const Duration(milliseconds: 300));
      await tester.pump();
    ...
    testWidgets('calendar explorer disables future dates', (tester) async {
      ...
      final container = ProviderScope.containerOf(tester.element(find.byType(ApodHistoryPage)));
      container.read(calendarMonthYearProvider.notifier).state = MonthYear(now.month, now.year + 1);
      await tester.pump(const Duration(milliseconds: 300));
      await tester.pump();
